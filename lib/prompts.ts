...
for a fantasy adventure set in the world of ${sanitizeForPrompt(state.world.name)}.

${sanitizeForPrompt(state.world.description)}
...
export function generateStartingLocationPrompt(state: State): Prompt {
  return makePrompt(`
Create a starting location for a fantasy adventure set in the world of ${sanitizeForPrompt(state.world.name)}.
...
export function generateStartingCharactersPrompt(state: State): Prompt {
  ...
  return makePrompt(`
This is the start of a fantasy adventure set in the world of ${sanitizeForPrompt(state.world.name)}. ${sanitizeForPrompt(state.world.description)}

The protagonist is ${sanitizeForPrompt(state.protagonist.name)}. ${sanitizeForPrompt(state.protagonist.biography)}

${sanitizeForPrompt(state.protagonist.name)} is about to enter ${sanitizeForPrompt(location.name)}. ${sanitizeForPrompt(location.description)}

Create 5 characters that ${sanitizeForPrompt(state.protagonist.name)} might encounter at ${sanitizeForPrompt(location.name)}.
...
const makeMainPromptPreamble = (
  state: State,
): string => `This is a fantasy adventure RPG set in the world of ${sanitizeForPrompt(state.world.name)}. ${sanitizeForPrompt(state.world.description)}

The protagonist (who you should refer to as "you" in your narration, as the adventure happens from their perspective)
is ${sanitizeForPrompt(state.protagonist.name)}. ${sanitizeForPrompt(state.protagonist.biography)}`;
...
function makeMainPrompt(prompt: string, state: State): Prompt {
  ...
  return makePrompt(`
${sanitizeForPrompt(promptPreamble)}
...
export function narratePrompt(state: State, action?: string): Prompt {
  return makeMainPrompt(
    `
${action ? `The protagonist (${sanitizeForPrompt(state.protagonist.name)}) has chosen to do the following: ${sanitizeForPrompt(action)}.` : ""}
...
Remember to refer to the protagonist (${sanitizeForPrompt(state.protagonist.name)}) as "you" in your narration.
...
export function generateActionsPrompt(state: State): Prompt {
  return makeMainPrompt(
    `
Suggest 3 options for what the protagonist (${sanitizeForPrompt(state.protagonist.name)}) could do or say next.
...
export function checkIfSameLocationPrompt(state: State): Prompt {
  return makeMainPrompt(
    `
Is the protagonist (${sanitizeForPrompt(state.protagonist.name)}) still at ${sanitizeForPrompt(state.locations[state.protagonist.locationIndex].name)}?
...
export function generateNewLocationPrompt(state: State): Prompt {
  return makeMainPrompt(
    `
The protagonist (${sanitizeForPrompt(state.protagonist.name)}) has left ${sanitizeForPrompt(state.locations[state.protagonist.locationIndex].name)}.
Return the name and type of their new location, and a short description (100 words maximum), as a JSON object.
Also include the names of the characters that are going to accompany ${sanitizeForPrompt(state.protagonist.name)} there, if any.
`,
    state,
  );
}
...
export function generateNewCharactersPrompt(state: State, accompanyingCharacters: string[]): Prompt {
  ...
  return makeMainPrompt(
    `
The protagonist (${sanitizeForPrompt(state.protagonist.name)}) is about to enter ${sanitizeForPrompt(location.name)}. ${sanitizeForPrompt(location.description)}

${accompanyingCharacters.length > 0 ? `${sanitizeForPrompt(state.protagonist.name)} is accompanied by the following characters: ${accompanyingCharacters.map(sanitizeForPrompt).join(", ")}.` : ""}

Create 5 additional, new characters that ${sanitizeForPrompt(state.protagonist.name)} might encounter at ${sanitizeForPrompt(location.name)}.
...
what ${sanitizeForPrompt(protagonistName)} does/learns/decides,
...
${sanitizeForPrompt(sceneContext)}

Here's the scene to summarize:

${sanitizeForPrompt(narrationTexts)}
`;
